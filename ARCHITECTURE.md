...
> **Purpose:** how code-quorum is laid out and why: the structure, the main pieces and how they fit, the data flow, the choices behind them. **Use:** read before changing structure; update in the same PR as a layout or design change; a reversed choice gets a record in docs/adr/.
...
- **opencode** (neutral) — `opencode run` subprocess (DeepSeek via OpenRouter), run in a sandboxed `HOME` with a generated read-only `council.md`. Two unbounded-growth/stall guards sit on this seat, both traced to upstream behaviour rather than guessed at. (1) The generated config **arms opencode's own stream watchdog** (`provider.openrouter.options.chunkTimeout`, in milliseconds): on the openai-compatible path that watchdog is unset by default, so a silently dropped SSE stream hangs forever — and because opencode's retries are error-driven, a stall that never errors never retries. Set below the seat's own idle timeout so opencode's internal retry gets its chance before our watchdog gives up. (2) `scripts/prune_opencode_db.py` caps the sandbox database, which nothing else bounds (measured at 516 MB after a couple of months, mostly message `part` rows and the `event` log). It deletes **only `agent = 'council'` sessions**, oldest first, keeping the newest few because the seat recovers a dropped answer by reading it back out of that database. A third piece, `quorum/agents/openrouter_relay.py`, exists for auditability rather than robustness: the seat pins an OpenRouter provider order, but opencode drops OpenRouter's `provider` field from its events and its database, so `run()` starts a **loopback relay** per run and hands its URL to that one opencode process through `OPENCODE_CONFIG_CONTENT` (a per-process config overlay opencode deep-merges over the sandbox file; the shared file never carries a port, so overlapping runs cannot cross-wire). The relay forwards each request unchanged, streams the response back chunk by chunk (write and drain per chunk, no read timeout of its own, so the seat's timing guards are unchanged), and appends metadata only (backend, generation id, tokens, cost, status, timings) to `~/.cache/code-quorum/opencode-served.jsonl`. It is best-effort: a failed bind or `CODE_QUORUM_OPENCODE_RELAY=0` runs the seat direct. After the run the seat logs the backend summary and warns when a backend outside the pin served a request. The pin is also sent as OpenRouter `only`, and the subprocess environment carries `OPENCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX` for pinned models, because opencode otherwise sends a fixed 32,000-token `max_tokens` (docs/adr/0002).
...
- **Byte caps with graceful degradation.** Plan files, scope docs, and review diffs cap at 100 KB; an over-cap diff degrades to a `git diff --stat` / gh-metadata summary that points agents at the working tree — or at `git show <sha>:<path>` when the working tree is known not to hold the reviewed change — rather than failing the run.

- **PR and range reviews warn when cwd diverges from the reviewed head.** `resolve_review_target` compares the working tree's `HEAD` against the reviewed PR/range's head sha; a mismatch (or an unresolvable head) prepends a notice telling the seat to judge the diff text, not the tree, since a seat that opens a file "to verify" against a stale checkout can otherwise report the diff's own additions as missing.
